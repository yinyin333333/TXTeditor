export const GAME_VERSIONS = ["3.3", "3.2", "3.1", "2.4", "1.13c"];
...
export function vectorSchemaForGameVersion(version) {
  return VECTOR_SCHEMAS[normalizeGameVersion(version)] ?? "3.3";
}
...
export function legacyGameVersion(config = {}, profile = "RotW") {
  ...
  if (reference === "3.1" || reference === "3.2" || reference === "3.3") return reference;
  const schema = normalizeGameVersion(config.schemaVersion);
  return schema === "3.1" || schema === "3.2" || schema === "3.3" ? schema : "3.3";
}
...
export function vectorGameVersion(config = {}) {
  ...
  return normalizeGameVersion(config.referenceVersion) ?? "3.3";
}

export function canonicalGameVersionConfig(config = {}, version) {
  const gameVersion = normalizeGameVersion(version) ?? "3.3";
  return {
    ...config,
    gameVersion,
