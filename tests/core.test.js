...
test("version metadata is bumped to TXTeditor 0.4.5", () => {
  ...
  assert.match(pkg.description, /TXTeditor 0\.4\.5/);
  assert.match(cargoToml, /version = "0\.4\.5"/);
  assert.match(cargoToml, /description = "TXTeditor 0\.4\.5/);
  assert.match(cargoLock, /name = "txteditor"\r?\nversion = "0\.4\.5"/);
  assert.equal(tauri.version, "0.4.5");
  assert.match(readme, /TXTeditor 0\.4\.5 is/);
});
