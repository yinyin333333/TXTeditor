...
test("release metadata remains 0.5.3 while the README introduction stays version-independent", () => {
  ...
  assert.equal(lock.packages[""].version, "0.5.3");
  assert.match(pkg.description, /TXTeditor 0\.5\.3/);
  assert.match(cargoToml, /version = "0\.5\.3"/);
  assert.match(cargoToml, /description = "TXTeditor 0\.5\.3/);
  assert.match(cargoLock, /name = "txteditor"\r?\nversion = "0\.5\.3"/);
  assert.equal(tauri.version, "0.5.3");
  assert.match(readme, /^# TXTeditor\r?\n\r?\nTXTeditor is a Windows-focused desktop editor/);
  assert.doesNotMatch(readme, /TXTeditor 0\.5\.3 is/);
});
