...
test("release metadata remains 0.5.2 while the README introduction stays version-independent", () => {
  ...
  assert.equal(lock.packages[""].version, "0.5.2");
  assert.match(pkg.description, /TXTeditor 0\.5\.2/);
  assert.match(cargoToml, /version = "0\.5\.2"/);
  assert.match(cargoToml, /description = "TXTeditor 0\.5\.2/);
  ...
  assert.doesNotMatch(readme, /TXTeditor 0\.5\.2 is/);
});
