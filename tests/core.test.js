...
test("version metadata is bumped to TXTeditor 0.4.6", () => {
  ...
  assert.match(pkg.description, /TXTeditor 0\.4\.6/);
  assert.match(cargoToml, /version = "0\.4\.6"/);
  assert.match(cargoToml, /description = "TXTeditor 0\.4\.6/);
  assert.match(cargoLock, /name = "txteditor"\r?\nversion = "0\.4\.6"/);
  assert.equal(tauri.version, "0.4.6");
  assert.match(readme, /TXTeditor 0\.4\.6 is/);
});
